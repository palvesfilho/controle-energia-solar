...
import { UsinaDaUc, usinaExibida } from "@/components/consumer-units/usina-da-uc";
...
interface UCData {
  ...
  /** Campo do CADASTRO — não diz de onde a UC compensa; ver `vinculo`. */
  plant: { id: string; name: string } | null;
  /** Usina pelos rateios (vigente / pendente / rejeitado) — lib/uc-vinculo-rateio.ts. */
  vinculo: VinculoUc;
  /** Fase derivada das faturas — ver lib/uc-implantacao.ts. */
  implantacao: FaseImplantacao | null;
}

type SortKey = "nome" | "codigoUc" | "usina" | "consumo" | "status" | "espera";
...
const FACETAS: Faceta<UCData>[] = [
  ...
  { chave: "usina", label: "Usina", valor: (u) => usinaExibida(u.vinculo) ?? "Sem usina" },
...
function UnidadesConsumidorasConteudo() {
  ...
  const filtro = useFiltroTabela(ucsDaAba, {
    sincronizarUrl: true,
    busca: (u) => [u.nome, u.codigoUc, u.consumer?.name, usinaExibida(u.vinculo), u.cidade],
    facetas: FACETAS,
  });
  ...
  const filtered = useMemo(() => {
    ...
    rows.sort((a, b) => {
      ...
      switch (sort.key) {
        ...
        case "usina": {
          // Sem usina vira "" e cai no TOPO do crescente: é o grupo que se
          // procura ao ordenar por aqui. Empate (mesma usina) vai pelo nome da UC.
          const ua = usinaExibida(a.vinculo) ?? "";
          const ub = usinaExibida(b.vinculo) ?? "";
          return (ua.localeCompare(ub, "pt-BR") || a.nome.localeCompare(b.nome, "pt-BR")) * dir;
        }
        case "consumo":
              ...
              <table className="w-full text-sm" data-tabela="unidades-consumidoras">
                <thead>
                  <tr className="border-b text-muted-foreground">
                    ...
                    <SortHeader
                      label="Usina"
                      active={sort.key === "usina"}
                      dir={sort.dir}
                      onClick={() => toggleSort("usina")}
                ...
                <tbody>
                  {filtered.map((uc) => (
                    ...
                    >
                      ...
                      <td className="py-2.5 px-3">
                        <UsinaDaUc vinculo={uc.vinculo} plantCadastro={uc.plant} active={uc.active} />
                      </td>
