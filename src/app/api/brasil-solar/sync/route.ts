...
// POST /api/brasil-solar/sync - Sincronizar plantas Fronius â†’ BrasilSolarClient
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    // Buscar clientes jÃ¡ existentes indexados por monitoramentoPlantId
    ...
    // Processar em lotes de 50 para nÃ£o sobrecarregar o banco
    const BATCH_SIZE = 50;
...
function mapFroniusToClient(sys: FroniusPvSystem) {
  // Extrair UF do endereÃ§o ou CEP (padrÃ£o BR)
  ...
  return {
    ...
    uf,
    potenciaInstalada: sys.peakPower ? sys.peakPower / 1000 : undefined, // W â†’ kWp
...
function extractUf(state: string | null, city: string | null): string | undefined {
  ...
  // Tentar extrair de padrÃµes comuns no nome da cidade
  if (city) {
