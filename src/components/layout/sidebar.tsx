    ...
    >
      ...
      <div className="flex h-16 items-center gap-3 px-4 border-b border-sidebar-border">
        <Link href="/portal" className="flex items-center gap-3 group">
          ...
          {!collapsed && (
            <span className="text-lg font-bold text-sidebar-foreground group-hover:text-sidebar-primary transition-colors">AURA</span>
          )}
        </Link>
      </div>

      {/* Módulo atual (só pra admin) */}
      {!simpleItems && !collapsed && (
      ...
      {!simpleItems && collapsed && (
        <div className="px-3 pt-3">
          <Link
            href="/portal"
            title={`Módulo: ${moduleMeta.label} — clique para trocar`}
            className={cn("flex h-9 w-9 mx-auto items-center justify-center rounded-md bg-gradient-to-br text-white", moduleMeta.accent)}
          >
