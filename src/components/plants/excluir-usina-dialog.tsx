...
// Diálogo de exclusão de usina — mesmo padrão do "Excluir investidor":
// dupla proteção (frase digitada + bloqueio por vínculos), mas aqui o impacto
// é consultado na API assim que abre, pra o operador ver o que some junto.
export function ExcluirUsinaDialog({
  ...
  useEffect(() => {
    ...
    fetch(`/api/plants/${plantId}/exclusao`)
  ...
  async function confirmarExclusao() {
    if (!plantId || !fraseBate || bloqueado) return;
    setDeleting(true);
    try {
      ...
      if (!res.ok) {
        if (res.status === 409 && Array.isArray(data.details)) {
          ...
          toast.error(data.error ?? "Falha ao excluir usina");
        }
        return;
      }

      toast.success(`Usina ${plantName} excluída`);
      onExcluida(plantId);
      onOpenChange(false);
    } catch (err) {
    ...
    <Dialog open={open} onOpenChange={(o) => !o && fechar()}>
      <DialogContent className="max-w-md">
        <DialogHeader>
          ...
          <DialogTitle className="text-center text-lg">Excluir usina?</DialogTitle>
        ...
        {carregandoImpacto ? (
          ...
          <div className="rounded-lg border border-red-200 bg-red-50 dark:border-red-900 dark:bg-red-950/40 px-3 py-2 text-sm">
            <div className="font-medium text-red-700 dark:text-red-300">
              Esta usina tem histórico e não pode ser excluída:
            </div>
