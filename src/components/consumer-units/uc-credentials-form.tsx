...
import { KeyRound, RefreshCw, Trash2, Mail, Hash, Building2, Clock, Pencil, Eye, EyeOff, Lock, Copy } from "lucide-react";
...
export function UcCredentialsForm({ consumerUnitId, defaultInstalacao, onSyncComplete }: UcCredentialsFormProps) {
  ...
  // Só exibição: a API deriva do cadastro da UC e devolve pronta.
  ...
  async function buscarSenhaSalva(): Promise<string | null> {
    ...
    setRevelando(true);
    try {
      const res = await fetch(`/api/consumer-units/${consumerUnitId}/credentials/senha`);
      const data = await res.json();
      if (!res.ok) {
  ...
  async function handleSync() {
    setSyncing(true);
    toast.info(
      distribuidora ? `Consultando faturas na ${distribuidora}...` : "Consultando faturas...",
    );
              ...
              <div className="grid grid-cols-2 gap-4">
                ...
                <div className="flex items-start gap-3">
                  ...
                  <div className="min-w-0">
                    ...
                    {/* Vem do cadastro da UC — não se escolhe aqui. */}
        ...
        {showForm && (
          <form onSubmit={handleSave} className="space-y-4">
            ...
            {/* A concessionária não é perguntada aqui: é sempre a do cadastro da
                UC. Perguntar duas vezes deixava os dois valores discordarem. */}
            {distribuidora && (
              <p className="text-xs text-muted-foreground">
