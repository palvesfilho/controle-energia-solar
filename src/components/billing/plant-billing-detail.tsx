...
interface UcCompensacao {
  ...
  /** Valor da compensação da UC no rateio (bruto + ajuste), SEM descontar
   *  InvestorDebit. É o que o card "Resumo do faturamento mensal" exibe. */
...
export interface PlantBillingDetailProps {
  /** ID do PlantBilling a exibir. */
  billingId: string;
  /** Mês no formato YYYY-MM — usado só no link "Voltar" da rota antiga. */
  mesParam?: string;
  /**
   * Embutido dentro da matriz (/admin/faturamento/usinas): esconde o link
   * "Voltar" e solta a largura máxima, já que a navegação fica com a régua.
   */
  embedded?: boolean;
  /**
   * Disparado sempre que os dados são recarregados após uma ação (pagamento,
   * upload, publicação, encerramento). A matriz usa pra repintar a célula.
   */
  onChanged?: () => void;
}

export function PlantBillingDetail({
  billingId,
  mesParam,
  embedded = false,
  onChanged,
}: PlantBillingDetailProps) {
  const id = billingId;
  const { user } = useUser();
  const userRole = ((user?.publicMetadata as Record<string, unknown> | undefined)?.role as string) ?? "";
  ...
  // Guardado em ref pra não entrar nas dependências do efeito — o pai pode
  // recriar o callback a cada render sem provocar refetch em loop.
  const onChangedRef = useRef(onChanged);
  onChangedRef.current = onChanged;

  /** `silent` = carga inicial; não avisa o pai (nada mudou ainda). */
  const reload = async (silent = false) => {
    const r = await fetch(`/api/billing/plants/${id}`);
    if (r.ok) {
      const d = await r.json();
      setData(d);
      if (!silent) onChangedRef.current?.();
    }
  };
  ...
  return (
    <div className={embedded ? "space-y-4" : "space-y-4 max-w-5xl"}>
      {!embedded && (
        <Link
          href={`/admin/faturamento/usinas${mesParam ? `/${mesParam}` : ""}`}
                  ...
                  {data.ucsCompensacao.map((uc) => {
                    ...
                    return (
                      ...
                      >
                        ...
                        <td className="px-3 py-2 text-right tabular-nums">
                          {uc.valorCompensacao != null ? formatBRL(uc.valorCompensacao) : "—"}
                        </td>
                        <td className="px-3 py-2 text-right tabular-nums">
                          {uc.valorPago > 0 ? (
                            <div>
